...
public:
...
#ifdef __ENABLE_MRUBY_PROC
  virtual struct RProc * procNew(mrb_irep *irep) = 0;
  virtual struct RProc * procNewCfunc(mrb_func_t func) = 0;
  virtual struct RProc * closureNew(mrb_irep *irep) = 0;
  virtual struct RProc * closureNewCfunc(mrb_func_t func, int nlocals) = 0;
  ...
  //virtual mrb_value strLiteral(mrb_value s) = 0;
