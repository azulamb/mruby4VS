...
public:
  ...
  // proc.c
  virtual struct RProc * procNew(mrb_irep *irep);
  virtual struct RProc * procNewCfunc(mrb_func_t func);
  virtual struct RProc * closureNew(mrb_irep *irep);
  virtual struct RProc * closureNewCfunc(mrb_func_t func, int nlocals);
  ...
  //virtual mrb_value strLiteral(mrb_value s);
